...
export default function App() {
  ...
  const handlePublishDownload = useCallback(()=>{
    ...
    // __HTML_TEMPLATE__ is set in a plain <script> block outside Babel,
    // so it has no escaping issues and is always available on window.
    const tmpl = window.__HTML_TEMPLATE__;
    if(!tmpl){ alert("HTML template not found. This feature requires the standalone HTML version."); return; }
    const dataJson = JSON.stringify(publishedData);
    // Inject data and stub out sample BASE_DATA
    const STUB = '{"meta":{"client":"","version":"1.0","data_model_version":"0.2"},"framework_config":{"impact_factors":[],"readiness_factors":[]},"implementation_enablers":[],"use_cases":[]}';
    const html = tmpl
      .replace('@@PUBLISHED_DATA@@', dataJson)
      .replace('@@BASE_DATA_STUB@@', STUB);
