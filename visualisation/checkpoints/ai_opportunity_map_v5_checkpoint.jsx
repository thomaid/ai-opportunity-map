...
function ScatterPlot({ computedUCs, colorMode, colorMap, highlightedUCIds, legendFilter, selectedUC, onSelectUC, T, enablerMap, showArrows, onSvgRef }) {
  ...
  useEffect(()=>{ if(onSvgRef) onSvgRef(svgRef); },[onSvgRef]);
...
function DetailPanel({uc,scores,impactFactors,readinessFactors,enablerMap,allUseCases,onSelectUC,T}) {
  if(!uc) return (
  ...
  return (
    <div>
      ...
      {(()=>{
        const enables   = (uc.enables||[]).map(id=>allUseCases?.find(u=>u.id===id)).filter(Boolean);
        const enabledBy = (allUseCases||[]).filter(u=>(u.enables||[]).includes(uc.id));
        if(enables.length===0 && enabledBy.length===0) return null;
        return (
          <>
            {enables.length>0&&(
              <>
                <div style={{fontSize:10,color:T.textDim,margin:"10px 0 5px",fontWeight:600,letterSpacing:0.4}}>
                  ENABLES
                </div>
                {enables.map(u=>(
                  <button key={u.id} onClick={()=>onSelectUC&&onSelectUC(u.id)}
                    style={{display:"flex",alignItems:"center",gap:8,width:"100%",
                            marginBottom:3,padding:"5px 7px",borderRadius:5,cursor:"pointer",
                            background:T.surface2,border:`1px solid ${T.border}`,
                            textAlign:"left",transition:"border-color 0.15s"}}
                    onMouseEnter={e=>e.currentTarget.style.borderColor="#3B82F6"}
                    onMouseLeave={e=>e.currentTarget.style.borderColor=T.border}>
                    <div style={{width:18,height:18,borderRadius:"50%",background:"#3B82F6",
                                 display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      <span style={{fontSize:8,fontWeight:700,color:"#fff"}}>{u.id.replace("UC","")}</span>
                    </div>
                    <span style={{fontSize:10,color:T.textMid,flex:1}}>{u.name}</span>
              ...
              <>
                ...
                {enabledBy.map(u=>(
                  <button key={u.id} onClick={()=>onSelectUC&&onSelectUC(u.id)}
                    ...
                    onMouseLeave={e=>e.currentTarget.style.borderColor=T.border}>
                    <div style={{width:18,height:18,borderRadius:"50%",background:"#8B5CF6",
                                 display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      <span style={{fontSize:8,fontWeight:700,color:"#fff"}}>{u.id.replace("UC","")}</span>
                    </div>
                    <span style={{fontSize:10,color:T.textMid,flex:1}}>{u.name}</span>
...
function ColorLegend({colorMode,colorMap,legendFilter,onToggleLegendFilter,T}) {
  ...
  return (
    <div style={{padding:"8px 16px",borderTop:`1px solid ${T.border}`,background:T.surface,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",minHeight:42,flexShrink:0}}>
...
function ModalShell({onClose,width=480,children,T}) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,
                 display:"flex",alignItems:"center",justifyContent:"center"}}
         onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{width,maxHeight:"80vh",display:"flex",flexDirection:"column",
...
function ImportConfirmModal({onConfirm,onCancel,T}) {
  return (
    <ModalShell onClose={onCancel} width={420} T={T}>
      <div style={{padding:"20px 22px 16px"}}>
        <div style={{fontSize:14,fontWeight:700,color:T.text,marginBottom:8}}>Load new dataset?</div>
        <div style={{fontSize:12,color:T.textMid,lineHeight:1.6}}>
          This will replace the current dataset and discard any unsaved session changes. This cannot be undone.
        </div>
      </div>
      <div style={{padding:"12px 22px 18px",display:"flex",justifyContent:"flex-end",gap:8}}>
        <button onClick={onCancel} style={{padding:"7px 16px",borderRadius:7,fontSize:12,
          border:`1px solid ${T.border2}`,background:"transparent",color:T.textMid,cursor:"pointer"}}>
          Cancel
        </button>
        <button onClick={onConfirm} style={{padding:"7px 18px",borderRadius:7,fontSize:12,
          border:"none",background:"#EF4444",color:"#fff",cursor:"pointer",fontWeight:600}}>
          Replace & load
        </button>
      </div>
    </ModalShell>
...
function PublishModal({onDownload,onClose,clientName,hasChanges,T}) {
  return (
    <ModalShell onClose={onClose} width={460} T={T}>
      <div style={{padding:"20px 22px 4px",borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
        <div style={{fontSize:14,fontWeight:700,color:T.text,marginBottom:4}}>Publish to client file</div>
        <div style={{fontSize:11,color:T.textDim,paddingBottom:16,lineHeight:1.6}}>
          Creates a standalone HTML file pre-loaded with the current dataset
          {hasChanges?" and all session adjustments baked in":" (no session adjustments pending)"}.
          Sample data is not included. The client opens this file directly in a browser.
        </div>
      </div>
      <div style={{padding:"16px 22px"}}>
        <div style={{padding:"12px 14px",background:T.surface2,borderRadius:8,
                     border:`1px solid ${T.border}`,marginBottom:12}}>
          <div style={{fontSize:10,color:T.textDim,marginBottom:3}}>CLIENT</div>
          <div style={{fontSize:13,color:T.text,fontWeight:600}}>{clientName||"—"}</div>
        </div>
        <div style={{fontSize:11,color:T.textDim,lineHeight:1.5}}>
          All interactive features remain available in the published file.
        </div>
      </div>
      <div style={{padding:"12px 22px 18px",display:"flex",justifyContent:"flex-end",gap:8,
                   borderTop:`1px solid ${T.border}`}}>
        <button onClick={onClose} style={{padding:"7px 16px",borderRadius:7,fontSize:12,
          border:`1px solid ${T.border2}`,background:"transparent",color:T.textMid,cursor:"pointer"}}>
          Cancel
        </button>
        <button onClick={onDownload} style={{padding:"7px 18px",borderRadius:7,fontSize:12,
          border:"none",background:"#22C55E",color:"#fff",cursor:"pointer",fontWeight:600}}>
...
export default function App() {
  ...
  // Active dataset: loaded client data if present, otherwise sample
  // (baseData is kept in sync with loadedData via resetAllState)
  ...
  const [loadedData, setLoadedData] = useState(null); // null = using sample BASE_DATA
  const svgRefHolder = useRef(null);
  const handleSvgRef = useCallback((ref)=>{ svgRefHolder.current = ref; },[]);

  const resetAllState = useCallback((newBase)=>{
    setBaseData(newBase);
    setSession({completionLevels:{},impactWeights:{},readinessWeights:{}});
    setSelectedUC(null);
    setHighlightedEnablers(new Set());
    setLegendFilter(new Set());
    setColorMode("investment_theme");
    setActivePanel("enablers");
    setShowArrows(false);
  },[]);

  const handleDownloadSVG = useCallback(()=>{
    const svgEl = svgRefHolder.current?.current;
    if(!svgEl) return;
    // Clone the SVG and embed a style block for fonts
    const clone = svgEl.cloneNode(true);
    clone.setAttribute("xmlns","http://www.w3.org/2000/svg");
    clone.setAttribute("xmlns:xlink","http://www.w3.org/1999/xlink");
    // Inline a simple font declaration
    const style = document.createElementNS("http://www.w3.org/2000/svg","style");
    style.textContent = "text { font-family: 'IBM Plex Sans', 'DM Sans', system-ui, sans-serif; }";
    ...
    a.href = url;
    a.download = `ai_opportunity_map_${new Date().toISOString().slice(0,10)}.svg`;
  ...
  // ── Import: load a JSON file produced by the extractor ────────────────────
  const fileInputRef = useRef(null);
  const [importConfirmPending, setImportConfirmPending] = useState(null); // holds parsed JSON while awaiting confirm
  ...
  const handleFileChange = useCallback((e)=>{
    ...
    reader.onload = (evt)=>{
      try {
        ...
        // Basic validation
        if(!parsed.framework_config || !parsed.use_cases)
          throw new Error("File does not appear to be a valid AI Opportunity Map dataset.");
        if(hasChanges || loadedData) {
          setImportConfirmPending(parsed);
        } else {
          setLoadedData(parsed);
          resetAllState(parsed);
        }
      } catch(err) {
  ...
  },[hasChanges, loadedData, resetAllState]);

  const confirmImport = useCallback(()=>{
    if(!importConfirmPending) return;
    setLoadedData(importConfirmPending);
    resetAllState(importConfirmPending);
    setImportConfirmPending(null);
  },[importConfirmPending, resetAllState]);

  const cancelImport = useCallback(()=>setImportConfirmPending(null),[]);

  // ── Publish: bake session changes into data, inject into HTML skeleton ──────
  const [publishModalOpen, setPublishModalOpen] = useState(false);
  const canPublish = !!loadedData; // greyed out until real data loaded

  const buildPublishedData = useCallback(()=>{
    // Deep clone baseData and bake in session changes permanently
    const d = JSON.parse(JSON.stringify(baseData));
    for(const en of d.implementation_enablers)
      if(session.completionLevels[en.id]!==undefined) en.completion_level=session.completionLevels[en.id];
    for(const f of d.framework_config.impact_factors)
      if(session.impactWeights[f.id]!==undefined) f.weight=session.impactWeights[f.id];
    for(const f of d.framework_config.readiness_factors)
      if(session.readinessWeights[f.id]!==undefined) f.weight=session.readinessWeights[f.id];
    return d;
  },[baseData,session]);
  ...
  const handlePublishDownload = useCallback(()=>{
    const publishedData = buildPublishedData();
    // Read the HTML skeleton from the <script type="text/plain" id="html-skeleton"> block
    const skeletonEl = document.getElementById("html-skeleton");
    if(!skeletonEl){ alert("HTML skeleton not found. This feature requires the standalone HTML version."); return; }
    const skeleton = skeletonEl.textContent;
    // Stub out BASE_DATA with a minimal placeholder (sample data not shipped to client)
    const STUB = '{"meta":{"client":"","version":"1.0"},"framework_config":{"impact_factors":[],"readiness_factors":[]},"implementation_enablers":[],"use_cases":[]}';
    // Inject published dataset as loadedData via a small bootstrap script
    const injection = `
<script>
__PUBLISHED_DATA__ = ${JSON.stringify(publishedData)};
</scr`+`ipt>
`;
    // Replace the BASE_DATA stub marker and insert injection before closing body
    let html = skeleton
      .replace('__BASE_DATA_STUB__', STUB)
      .replace('</body>', injection + '</body>');
    const blob = new Blob([html], {type:"text/html"});
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const clientName = (publishedData.meta?.client||"client").replace(/[^a-z0-9]/gi,"_").toLowerCase();
    a.href=url; a.download=`ai_opportunity_map_${clientName}_${new Date().toISOString().slice(0,10)}.html`;
    ...
    <div style={{height:"100vh",background:T.bg,display:"flex",flexDirection:"column",fontFamily:"'IBM Plex Sans','DM Sans',system-ui,sans-serif",color:T.text,overflow:"hidden"}}>
      ...
      <div style={{padding:"10px 20px",borderBottom:`1px solid ${T.border}`,background:T.surface,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexShrink:0,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
          ...
          <button onClick={handleDownloadSVG} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${T.border2}`,background:T.surface2,color:T.textMid,cursor:"pointer",fontSize:11,transition:"all 0.15s"}} title="Download chart as SVG">
            ↓ SVG
          </button>
          {/* Import button */}
          <input ref={fileInputRef} type="file" accept=".json" onChange={handleFileChange} style={{display:"none"}}/>
          <button onClick={handleImportClick} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${T.border2}`,background:T.surface2,color:T.textMid,cursor:"pointer",fontSize:11,transition:"all 0.15s"}}>
            ↑ Load data
          </button>
          {/* Dataset indicator */}
          {loadedData&&<span style={{fontSize:10,color:"#22C55E",fontWeight:600,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={loadedData.meta?.client}>● {loadedData.meta?.client||"Dataset loaded"}</span>}
          ...
          {/* Publish button */}
          <button onClick={handlePublishClick} disabled={!canPublish} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${canPublish?"#22C55E":T.border2}`,background:canPublish?"rgba(34,197,94,0.15)":T.surface2,color:canPublish?"#22C55E":T.textDim,cursor:canPublish?"pointer":"default",fontSize:11,fontWeight:canPublish?600:400,opacity:canPublish?1:0.4,transition:"all 0.2s"}} title={canPublish?"Publish to client file":"Load a dataset first"}>
            ⬡ Publish
          </button>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",minHeight:0}}>
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0}}>
          <div style={{flex:1,position:"relative",minHeight:0,overflow:"hidden"}}>
            <ScatterPlot computedUCs={computedUCs} colorMode={colorMode} colorMap={colorMap} highlightedUCIds={highlightedUCIds} legendFilter={legendFilter} selectedUC={selectedUC} onSelectUC={id=>{setSelectedUC(id);if(id)setActivePanel("detail");}} T={T} enablerMap={enablerMap} showArrows={showArrows} onSvgRef={handleSvgRef}/>
          </div>
        ...
        {/* Right panel */}
        <div style={{width:300,borderLeft:`1px solid ${T.border}`,display:"flex",flexDirection:"column",background:T.surface,flexShrink:0,minHeight:0}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"12px 10px"}}>
            ...
            {activePanel==="detail"&&<DetailPanel uc={selectedUCData?.uc} scores={selectedUCData?.scores} impactFactors={baseData.framework_config.impact_factors} readinessFactors={baseData.framework_config.readiness_factors} enablerMap={enablerMap} allUseCases={baseData.use_cases} onSelectUC={setSelectedUC} T={T}/>}
          </div>
        </div>
      </div>

    </div>
    {importConfirmPending&&<ImportConfirmModal onConfirm={confirmImport} onCancel={cancelImport} T={T}/>}
    {publishModalOpen&&<PublishModal onDownload={handlePublishDownload} onClose={()=>setPublishModalOpen(false)} clientName={baseData.meta?.client} hasChanges={hasChanges} T={T}/>}
    </>
  );
}
